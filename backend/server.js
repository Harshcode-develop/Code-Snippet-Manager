...
app.post('/api/auth/google', async (req, res) => {
    try {
        ...
        // This log will tell you if the token is being received
...
app.post('/api/folders', authenticateToken, async (req, res) => {
    try {
        ...
        const [result] = await dbPool.query('INSERT INTO folders (user_id, name, parent_id) VALUES (?, ?, ?)', [userId, name, parentId || null]);
        res.status(201).json({ id: result.insertId, name, user_id: userId, parent_id: parentId || null, subfolders: [], snippets: [] });
    } catch (error) {
...
app.post('/api/snippets', authenticateToken, async (req, res) => {
    try {
        ...
        const [result] = await dbPool.query('INSERT INTO snippets (user_id, title, code, folder_id) VALUES (?, ?, ?, ?)', [userId, title, code, folderId || null]);
        res.status(201).json({ id: result.insertId, title, code, user_id: userId, folder_id: folderId || null });
    } catch (error) {
